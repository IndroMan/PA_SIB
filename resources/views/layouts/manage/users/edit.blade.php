                    ...
                    <div class="mb-3">
                        ...
                        <select id="select-role" class="form-select" name="role">
                            @foreach ($roles as $item)
                                <option value="{{ $item->name }}" @if ($user->roles->first()->name == $item->name) selected @endif>
                                    {{ Str::title($item->name) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="d-flex justify-content-end">
                        <button type="submit" class="btn btn-primary d-flex align-items-center">
                            <iconify-icon icon="solar:pen-linear" class="me-2" style="font-size:18px"></iconify-icon> Edit
                            User</button>
                    </div>
                </form>
            </div>
